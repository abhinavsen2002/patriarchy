...
def simulate_run(
    ...
    random_path = []
    sticky_path = []
    frames = [(traits.copy(), council.copy())] if record_frames else None
...
def animate_comparison(size: str, seed: int, out: Path) -> None:
    cfg = C.SIZES[size]
    conditions = (
        ("Baseline", 0.0, 0),
        ("Random ties only", 0.5, 0),
        ("Random ties + stickiness", 0.5, 5),
    )
    runs = [
        simulate_run(
            cfg["n"],
        ...
        for _, random_mix, sticky_years in conditions
    ]
    stride = max(1, N_YEARS // 60)
    frame_ids = list(range(0, N_YEARS + 1, stride))
    if frame_ids[-1] != N_YEARS:
        frame_ids.append(N_YEARS)
    fig = plt.figure(figsize=(14, 12), facecolor=C.DARK_BG)
    axes3d = [
        fig.add_subplot(3, 2, 2 * row + 1, projection="3d")
        for row in range(3)
    ]
    axes_path = [fig.add_subplot(3, 2, 2 * row + 2) for row in range(3)]
    C.style_dark_figure(fig, axes3d + axes_path)
    lines = []
    city_lines = []
    for row, ((label, _, _), run) in enumerate(zip(conditions, runs)):
        traits, council = run["frames"][0]
        S4.scatter_merit(
            axes3d[row], traits, council, cfg["n"], dark=True
        )
        axes3d[row].set_title(f"{label} — year 0")
        line, = axes_path[row].plot(
            [], [], color=C.C_BLUE_BRIGHT, lw=2.6
        )
        lines.append(line)
        ...
        axes_path[row].axhline(
            50, color=C.DARK_MUTED, ls=":", alpha=0.65
        )
        axes_path[row].set_xlim(0, N_YEARS)
        axes_path[row].set_ylim(0, 100)
        axes_path[row].set_xlabel("Year")
        axes_path[row].set_ylabel("% council blue")
        legend = axes_path[row].legend(
            (line, city_line), ("council", "whole city"), frameon=False
        )
        C.style_dark_legend(legend)
    fig.suptitle(
        f"Scene 5 — do random ties need stickiness?  ·  N={cfg['n']}",
        color=C.DARK_TEXT,
        fontsize=14,
        fontweight="bold",
    )
    ...
    def update(frame_index):
        ...
        for row, ((label, _, _), run) in enumerate(zip(conditions, runs)):
            traits, council = run["frames"][year]
            axes3d[row].cla()
            S4.scatter_merit(
                axes3d[row], traits, council, cfg["n"], dark=True
            )
            axes3d[row].set_title(f"{label} — year {year}")
            lines[row].set_data(np.arange(year + 1), run["blue"][: year + 1])
            city_lines[row].set_data(
                np.arange(year + 1), run["city_blue"][: year + 1]
            )
        return tuple(lines + city_lines)

    animation = FuncAnimation(
        fig, update, frames=len(frame_ids), interval=120, blit=False
    )
    C.save_mp4(animation, out, fps=10)
    plt.close(fig)
...
def main() -> None:
    ...
    for size in C.sizes_to_run(args.size):
        ...
        if args.animate:
            animate_comparison(
                size, args.seed, HERE / f"scene5_n{results['n']}.mp4"
            )
